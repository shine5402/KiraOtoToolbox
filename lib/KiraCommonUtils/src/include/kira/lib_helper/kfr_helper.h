...
template <typename T = void> struct qt_file_writer : abstract_writer<T>
{
    qt_file_writer(std::unique_ptr<QFile> &&device) : device(std::move(device)) {}
    ~qt_file_writer() override {}

    using abstract_writer<T>::write;
    size_t write(const T *data, size_t size) final { return device->write(static_cast<const char *>(data), size); }
...
template <typename T = void> struct qt_file_reader : abstract_reader<T>
{
    qt_file_reader(std::unique_ptr<QFile> &&device) : device(std::move(device)) {}
    ~qt_file_reader() override {}
    size_t read(T *data, size_t size) final { return device->read(static_cast<char *>(data), size); }
...
inline bool audio_sample_is_float(audio_sample_type type)
{
    return type >= audio_sample_type::first_float;
}
...
template <typename sample_t> std::unique_ptr<sample_t[]> interleave(univector2d<sample_t> in, size_t &out_size)
{
    if (in.empty()) {
        out_size = 0;
        return nullptr;
    ...
    kfr::interleave(interleaveBuffer.get(), rawDatas.get(), in.size(),
                    in[0].size()); // in.size -> channel_count, in[0].size() -> length
    out_size = totalSampleCount;
    return interleaveBuffer;
}
template <typename T>
size_t write_mutlichannel_wav_file(kfr::audio_writer_wav<T> &writer, const kfr::univector2d<T> &data,
                                   size_t *to_write_p = nullptr)
{
