...
private slots:
    ...
    void construct_validTest_data() { prepareNormalData(); }

    void construct_fileNameTest();
    void construct_fileNameTest_data() { prepareNormalData(); }

    void construct_aliasTest();
    void construct_aliasTest_data() { prepareNormalData(); }

    void construct_leftTest();
    void construct_leftTest_data() { prepareNormalData(); }

    void construct_consonantTest();
    void construct_consonantTest_data() { prepareNormalData(); }

    void construct_rightTest();
    void construct_rightTest_data() { prepareNormalData(); }

    void construct_preUtteranceTest();
    void construct_preUtteranceTest_data() { prepareNormalData(); }

    void construct_overlapTest();
    void construct_overlapTest_data() { prepareNormalData(); }

    void construct_errorTest();
    void construct_errorTest_data();

    void toStringTest();
    void toStringTest_data() { prepareNormalData(); }
    ...
    void getParameterTest_data() { prepareNormalData(); }
...
void OtoUtilTest::prepareNormalData()
{
    ...
    QTest::newRow("VC") << "_さんささすぃさすさ.wav=a sA#3,2108.431,281.383,-320.601,230.769,76.922"
                        << "_さんささすぃさすさ.wav" << "a sA#3" << 2108.431 << 281.383 << -320.601 << 230.769
                        << 76.922;
    QTest::newRow("End Breath with Chinese Character")
...
void OtoUtilTest::construct_errorTest_data()
{
    ...
    QTest::newRow("No filename seperator") << "あ.wav,149,201,-599,0,0" << OtoEntry::FileNameSeparatorNotFound
                                           << "The separator between fileName and alias are not found.";
    QTest::newRow("Wrong left") << "あ.wav=,14q9,201,-599,0,0" << OtoEntry::LeftConvertFailed
                                << "Convert left string to double failed.";
    QTest::newRow("Wrong consonant") << "あ.wav=,149,2a01,-599,0,0" << OtoEntry::ConsonantConvertFailed
                                     << "Convert consonant string to double failed.";
    QTest::newRow("Wrong right") << "あ.wav=,149,201,-5right9,0,0" << OtoEntry::RightConvertFailed
                                 << "Convert right string to double failed.";
    QTest::newRow("Wrong preUtterance") << "あ.wav=,149,201,-59,0preUtterance,0" << OtoEntry::PreUtteranceConvertFailed
                                        << "Convert preUtterance string to double failed.";
    QTest::newRow("Wrong overlap") << "あ.wav=,149,201,-59,0,0overlap" << OtoEntry::OverlapConvertFailed
                                   << "Convert overlap string to double failed.";
}
...
void OtoUtilTest::fileReader_test_data()
{
    ...
    QTest::newRow("normalData") << ":/file2test/normalData.ini" << 7
                                << QStringList{
                                       "_さんささすぃさすさ.wav=- さA#3,882.561,346.153,-538.461,230.769,76.923",
...
void OtoUtilTest::pitchStringRangeTest()
{
    ...
    auto result = OtoEntryFunctions::getPitchStringRange(bottom, top,
                                                         static_cast<OtoEntryFunctions::CharacterCase>(characterCase));
...
void OtoUtilTest::pitchStringRangeTest_data()
{
    ...
    QTest::newRow("Same octave") << "C3" << "F3" << static_cast<int>(OtoEntryFunctions::Upper)
                                 << QStringList{"C3", "C#3", "D3", "D#3", "E3", "F3"};
    QTest::newRow("Same octave (negative)") << "C-3" << "F-3" << static_cast<int>(OtoEntryFunctions::Upper)
                                            << QStringList{"C-3", "C#-3", "D-3", "D#-3", "E-3", "F-3"};
    QTest::newRow("Same octave (double digit)") << "C13" << "F13" << static_cast<int>(OtoEntryFunctions::Upper)
                                                << QStringList{"C13", "C#13", "D13", "D#13", "E13", "F13"};
    QTest::newRow("Different octave") << "D3" << "D4" << static_cast<int>(OtoEntryFunctions::Upper)
                                      << QStringList{"D3", "D#3", "E3", "F3", "F#3", "G3", "G#3",
                                                     "A3", "A#3", "B3", "C4", "C#4", "D4"};
    QTest::newRow("Different octave (negative to zero)")
        << "D-1" << "D0" << static_cast<int>(OtoEntryFunctions::Upper)
        << QStringList{"D-1", "D#-1", "E-1", "F-1", "F#-1", "G-1", "G#-1", "A-1", "A#-1", "B-1", "C0", "C#0", "D0"};

    QTest::newRow("Same octave, lower character") << "c3" << "f3" << static_cast<int>(OtoEntryFunctions::Lower)
                                                  << QStringList{"c3", "c#3", "d3", "d#3", "e3", "f3"};
    QTest::newRow("Same octave (negative), lower character")
        << "c-3" << "f-3" << static_cast<int>(OtoEntryFunctions::Lower)
        << QStringList{"c-3", "c#-3", "d-3", "d#-3", "e-3", "f-3"};
    QTest::newRow("Same octave (double digit), lower character")
        << "c13" << "f13" << static_cast<int>(OtoEntryFunctions::Lower)
        << QStringList{"c13", "c#13", "d13", "d#13", "e13", "f13"};
    QTest::newRow("Different octave, lower character")
        << "d3" << "d4" << static_cast<int>(OtoEntryFunctions::Lower)
        << QStringList{"d3", "d#3", "e3", "f3", "f#3", "g3", "g#3", "a3", "a#3", "b3", "c4", "c#4", "d4"};
    QTest::newRow("Different octave (negative to zero), lower character")
        << "d-1" << "d0" << static_cast<int>(OtoEntryFunctions::Lower)
        << QStringList{"d-1", "d#-1", "e-1", "f-1", "f#-1", "g-1", "g#-1", "a-1", "a#-1", "b-1", "c0", "c#0", "d0"};
}
...
void OtoUtilTest::removePitchSuffixTest()
{
    ...
    QCOMPARE(
        OtoEntryFunctions::removePitchSuffix(src, bottomPitch, topPitch, cs, characterCaseEnum, &pitchToRemoveResult),
        expect);
...
void OtoUtilTest::removePitchSuffixTest_data()
{
    ...
    QTest::newRow("case sensitive, upper")
        << "aF3" << "C1" << "C6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Upper) << "F3" << "a";
    QTest::newRow("case sensitive, upper, not match")
        << "af3" << "C1" << "C6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Upper) << "" << "af3";
    QTest::newRow("case insensitive, upper")
        << "af3" << "C1" << "C6" << Qt::CaseInsensitive << static_cast<int>(OtoEntryFunctions::Upper) << "F3" << "a";
    QTest::newRow("case sensitive, lower")
        << "af3" << "C1" << "C6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Lower) << "f3" << "a";
    QTest::newRow("case sensitive, lower, not match")
        << "aF3" << "C1" << "C6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Lower) << "" << "aF3";
    QTest::newRow("case insensitive, lower")
        << "aF3" << "C1" << "C6" << Qt::CaseInsensitive << static_cast<int>(OtoEntryFunctions::Lower) << "f3" << "a";

    QTest::newRow("case sensitive, upper, lower range input")
        << "aF3" << "c1" << "c6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Upper) << "F3" << "a";
    QTest::newRow("case sensitive, upper, not match, lower range input")
        << "af3" << "c1" << "c6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Upper) << "" << "af3";
    QTest::newRow("case insensitive, upper, lower range input")
        << "af3" << "c1" << "c6" << Qt::CaseInsensitive << static_cast<int>(OtoEntryFunctions::Upper) << "F3" << "a";
    QTest::newRow("case sensitive, lower, lower range input")
        << "af3" << "c1" << "c6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Lower) << "f3" << "a";
    QTest::newRow("case sensitive, lower, not match, lower range input")
        << "aF3" << "c1" << "c6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Lower) << "" << "aF3";
    QTest::newRow("case insensitive, lower, lower range input")
        << "aF3" << "c1" << "c6" << Qt::CaseInsensitive << static_cast<int>(OtoEntryFunctions::Lower) << "f3" << "a";

    QTest::newRow("range not match") << "aF3" << "A3" << "C4" << Qt::CaseSensitive
                                     << static_cast<int>(OtoEntryFunctions::Upper) << "" << "aF3";

    QTest::newRow("with #") << "aF#3" << "F3" << "G3" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Upper)
                            << "F#3" << "a";
}
...
void OtoUtilTest::removePitchPrefixTest()
{
    ...
    QCOMPARE(
        OtoEntryFunctions::removePitchPrefix(src, bottomPitch, topPitch, cs, characterCaseEnum, &pitchToRemoveResult),
        expect);
    QCOMPARE(pitchToRemoveResult, pitchToRemove);
}
...
void OtoUtilTest::removePitchPrefixTest_data()
{
    ...
    QTest::newRow("case sensitive, upper")
        << "F3a" << "C1" << "C6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Upper) << "F3" << "a";
    QTest::newRow("case sensitive, upper, not match")
        << "f3a" << "C1" << "C6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Upper) << "" << "f3a";
    QTest::newRow("case insensitive, upper")
        << "f3a" << "C1" << "C6" << Qt::CaseInsensitive << static_cast<int>(OtoEntryFunctions::Upper) << "F3" << "a";
    QTest::newRow("case sensitive, lower")
        << "f3a" << "C1" << "C6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Lower) << "f3" << "a";
    QTest::newRow("case sensitive, lower, not match")
        << "F3a" << "C1" << "C6" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Lower) << "" << "F3a";
    QTest::newRow("case insensitive, lower")
        << "F3a" << "C1" << "C6" << Qt::CaseInsensitive << static_cast<int>(OtoEntryFunctions::Lower) << "f3" << "a";
    ...
    QTest::newRow("range not match") << "F3a" << "A3" << "C4" << Qt::CaseSensitive
                                     << static_cast<int>(OtoEntryFunctions::Upper) << "" << "F3a";

    QTest::newRow("with #") << "F#3a" << "F3" << "G3" << Qt::CaseSensitive << static_cast<int>(OtoEntryFunctions::Upper)
                            << "F#3" << "a";
}
