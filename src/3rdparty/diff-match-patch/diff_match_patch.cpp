...
void diff_match_patch::diff_cleanupSemantic(QList<Diff> &diffs) {
    ...
    while (thisDiff) {
        if (prevDiff && prevDiff->operation == DELETE &&
...
void diff_match_patch::diff_cleanupSemanticLossless(QList<Diff> &diffs) {
    ...
    while (nextDiff) {
        if (thisDiff && prevDiff && prevDiff->operation == EQUAL &&
...
void diff_match_patch::diff_cleanupMerge(QList<Diff> &diffs) {
  ...
  */
    ...
    // Intentionally ignore the first and last element (don't need checking).
    while (nextDiff) {
        if (prevDiff && prevDiff->operation == EQUAL &&
                nextDiff->operation == EQUAL) {
            // This is a single edit surrounded by equalities.
            if (thisDiff && thisDiff->text.endsWith(prevDiff->text)) {
                ...
                nextDiff = pointer.hasNext() ? &pointer.next() : nullptr;
                changes = true;
            } else if (thisDiff && thisDiff->text.startsWith(nextDiff->text)) {
