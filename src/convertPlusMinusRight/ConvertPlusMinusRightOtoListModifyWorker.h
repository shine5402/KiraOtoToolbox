...
public:
    ...
    public:
        FileCannotReadException(const QString &fileName)
            : ToolException(tr("Cannot open file \"%1\", or it contains invalid data. Please check and try again.")
                                .arg(fileName)),
              fileName_(fileName) {};
        QString fileName() const { return fileName_; }
    ...
    public:
        InvalidRightValue(const OtoEntry &entry, double calculatedRight)
            : ToolException(
                  tr("Invalid right value\"%1\" has been calculated for oto \"%2\". Please check and try again.")
                      .arg(calculatedRight)
                      .arg(entry.toString())),
              entry_(entry), calculatedRight_(calculatedRight) {};

        OtoEntry entry() const { return entry_; };
        double calculatedRight() const { return calculatedRight_; }
