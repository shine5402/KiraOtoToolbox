...
OtoEntry::OtoEntry(QString fileName, QString alias, double left, double consonant, double right, double preUtterance,
                   double overlap, QObject *parent)
    : QObject(parent), fileName_(std::move(fileName)), alias_(std::move(alias)), left_(left), consonant_(consonant),
      right_(right), preUtterance_(preUtterance), overlap_(overlap), valid_(true)
{
}
...
{
    return fileName() == rhs.fileName() && alias() == rhs.alias() && left() == rhs.left() &&
           consonant() == rhs.consonant() && right() == rhs.right() && preUtterance() == rhs.preUtterance() &&
           overlap() == rhs.overlap();
}
...
void OtoEntry::setParameter(OtoEntry::OtoParameter parameter, QVariant value)
{
    ...
    case Left:
        ...
        break;
    case Consonant:
        setConsonant(value.toDouble());
        break;
    case Right:
        setRight(value.toDouble());
        break;
    case PreUtterance:
        setPreUtterance(value.toDouble());
        break;
    case Overlap:
        setOverlap(value.toDouble());
        break;
    }
}
    ...
    if (fileNameEndIndex != -1) {
        ...
        for (int i = parameterID::LEFT; i <= parameterID::OVERLAP; i++) {
            if (!doubleConvertOks.at(i)) {
                switch (i) {
                case LEFT:
                    setError(LeftConvertFailed);
                    break;
                case CONSONANT:
                    setError(ConsonantConvertFailed);
                    break;
                case RIGHT:
                    setError(RightConvertFailed);
                    break;
                case PREUTTERANCE:
                    setError(PreUtteranceConvertFailed);
                    break;
                case OVERLAP:
                    setError(OverlapConvertFailed);
                    break;
                }
                return result;
            }
...
QString OtoEntryFunctions::removePitchSuffix(QString alias, const QString &bottomPitch, const QString &topPitch,
                                             Qt::CaseSensitivity cs, CharacterCase pitchRangeCharacterCase,
                                             QString *pitchRemoved)
{
...
QString OtoEntryFunctions::removePitchPrefix(QString alias, const QString &bottomPitch, const QString &topPitch,
                                             Qt::CaseSensitivity cs,
                                             OtoEntryFunctions::CharacterCase pitchRangeCharacterCase,
                                             QString *pitchRemoved)
{
    auto pitchRange = getPitchStringRange(bottomPitch, topPitch, pitchRangeCharacterCase);
    for (const auto &pitch : pitchRange) {
...
QStringList OtoEntryFunctions::getPitchStringRange(const QString &bottomPitch, const QString &topPitch,
                                                   CharacterCase cs)
{
...
bool OtoEntryFunctions::writeOtoListToFile(const QString &fileName, const OtoEntryList &entryList, int precision,
                                           QFileDevice::FileError *error, QString *errorString, QTextCodec *textCodec,
