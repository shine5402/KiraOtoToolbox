...
public:
    Q_INVOKABLE OtoEntry(QString fileName, QString alias, double left, double consonant, double right,
                         double preUtterance, double overlap, QObject *parent = nullptr);
    ...
    public:
        ...
        OtoEntryError error() const { return error_; }
    ...
    enum OtoParameterOrder { FILENAME, ALIAS, LEFT, CONSONANT, RIGHT, PREUTTERANCE, OVERLAP };
...
Q_NAMESPACE
enum CharacterCase { Upper, Lower };
Q_ENUM_NS(CharacterCase)

QStringList getPitchStringRange(const QString &bottomPitch, const QString &topPitch,
                                CharacterCase characterCase = Upper);
QString removePitchSuffix(QString alias, const QString &bottomPitch, const QString &topPitch,
                          Qt::CaseSensitivity cs = Qt::CaseInsensitive,
                          CharacterCase pitchRangeCharacterCase = CharacterCase::Upper,
                          QString *pitchRemoved = nullptr);
QString removePitchPrefix(QString alias, const QString &bottomPitch, const QString &topPitch,
                          Qt::CaseSensitivity cs = Qt::CaseInsensitive,
                          CharacterCase pitchRangeCharacterCase = CharacterCase::Upper,
                          QString *pitchRemoved = nullptr);
QString removeSuffix(QString string, const QString &suffix, Qt::CaseSensitivity cs = Qt::CaseSensitive,
                     bool *removed = nullptr);
QString removePrefix(QString string, const QString &prefix, Qt::CaseSensitivity cs = Qt::CaseSensitive,
                     bool *removed = nullptr);
QString getDigitSuffix(const QString &string, int *position = nullptr, bool considerNegative = false);
QString removeDigitSuffix(QString string, int *position = nullptr, bool considerNegative = false,
                          bool *removed = nullptr);

bool writeOtoListToFile(const QString &fileName, const OtoEntryList &entryList,
                        int precision = OTOENTRY_DEFAULT_PRECISION, QFileDevice::FileError *error = nullptr,
                        QString *errorString = nullptr, QTextCodec *textCodec = QTextCodec::codecForName("Shift-JIS"),
