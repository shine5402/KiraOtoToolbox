...
void OverlapBatchSetOtoListModifyWorker::doWork(const OtoEntryList &srcOtoList, OtoEntryList &resultOtoList,
    ...
    if (options.getOption("ifSetOverlapStartWith").toBool()) {
        for (int i = 0; i < resultOtoList.count(); ++i) {
            ...
            for (const auto &s : std::as_const(patternList)) {
                if (entry.alias().startsWith(s) ||
                    (options.getOption("ifMatchStartOto").toBool() && entry.alias().startsWith("- " + s)))
                {
    ...
    if (options.getOption("makeOneThird").toBool()) {
        for (int i = 0; i < resultOtoList.count(); ++i) {
            if (!matched.contains(i)) {
                ...
                if (options.getOption("oneSecondWhenTooSmall").toBool() &&
                    result < options.getOption("oneSecondWhenTooSmallValue").toDouble())
