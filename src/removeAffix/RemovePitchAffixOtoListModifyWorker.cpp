...
void RemovePitchAffixOtoListModifyWorker::doWork(const OtoEntryList &srcOtoList, OtoEntryList &resultOtoList,
                                                 OtoEntryList &secondSaveOtoList, const OptionContainer &options)
{
    ...
    auto func = [&](RemovedStringInfo::AffixType affixType) {
        for (int i = 0; i < srcOtoList.count(); ++i) {
            ...
            auto result =
                removeFunc(currentOto.alias(), options.getOption("bottomPitch").toString(),
