...
OrganizeDuplicateOtoListModifyWorker::OrganizeDuplicateOtoListModifyWorker(QObject *parent)
    : OtoListModifyWorker(parent)
{
}

void OrganizeDuplicateOtoListModifyWorker::doWork(const OtoEntryList &srcOtoList, OtoEntryList &resultOtoList,
                                                  OtoEntryList &secondSaveOtoList, const OptionContainer &options)
{
    ...
    for (const auto &key : std::as_const(stringListUnique)) {
        ...
        for (int i = 0; i < values.count(); ++i) {
            ...
            newAlias.insert(
                currentID,
                compareStringList.at(currentID) +
