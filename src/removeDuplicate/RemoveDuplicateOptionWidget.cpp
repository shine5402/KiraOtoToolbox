...
RemoveDuplicateOptionWidget::RemoveDuplicateOptionWidget(QWidget *parent)
    : ToolOptionWidget(parent), ui(new Ui::RemoveDuplicateOptionWidget)
{
    ui->setupUi(this);

    connect(ui->maxSpinBox, qOverload<int>(&QSpinBox::valueChanged), this, &ToolOptionWidget::userSettingsChanged);
    connect(ui->affixRemoveWidget, &ToolOptionWidget::userSettingsChanged, this,
...
OptionContainer RemoveDuplicateOptionWidget::getOptions() const
{
    ...
    options.setOption("pitchCaseOrganized", ui->organizeCaseComboBox->currentIndex() == 0 ? OtoEntryFunctions::Upper
                                                                                          : OtoEntryFunctions::Lower);
...
void RemoveDuplicateOptionWidget::setOptions(const OptionContainer &options)
{
    ...
    ui->organizeCaseComboBox->setCurrentIndex(
        options.getOption("pitchCaseOrganized").toInt() == OtoEntryFunctions::Upper ? 0 : 1);
