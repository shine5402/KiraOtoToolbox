                                         ...
                                         OtoEntryList &secondSaveOtoList, const OptionContainer &options)
{
    ...
    for (int i = 0; i < srcOtoList.count(); ++i) {
        ...
        currentAlias.replace(
            "%a", shouldRemoveSuffix ? OtoEntryFunctions::removeSuffix(srcOtoList.at(i).alias(), digitSuffixList.at(i))
                                     : srcOtoList.at(i).alias());
    ...
    for (const auto &key : std::as_const(uniqueKeys)) {
        ...
        for (int i = 0; i < values.count(); ++i) {
            ...
            newAlias.insert(currentID, compareStringList.at(currentID) + (i > 0 ? QString::number(i + 1) : ""));
        }
    }
