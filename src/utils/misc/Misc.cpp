                                ...
                                ChangeAskDialogType changeType, QDialogButtonBox::StandardButtons stdButtons)
{
    ...
    case (Diff): {
        ...
        return new ShowDiffDialog(otoList2String(srcOtoList, precision), otoList2String(resultOtoList, precision),
                                  title, label, stdButtons, dialogParent);
    }
    case (Determine):
        Q_UNREACHABLE();
    }
...
{
    return getOtoDiffDialog(srcOtoList, resultOtoList, precision, title, label, dialogParent, changeType, stdButtons)
        ->exec();
}

bool Misc::askUserWithShowOtoList(const OtoEntryList &secondSaveData, const QString &title, const QString &label,
...
QTextCodec *Misc::detectCodecAndAskUserIfNotShiftJIS(const QString &path, QWidget *dialogParent)
{
    ...
    if (file.open(QFile::Text | QFile::ReadOnly)) {
        ...
        int bytes_consumed;
        bool is_reliable;
        auto detectedEncoding = CompactEncDet::DetectEncoding(
            content.c_str(), content.size(), nullptr, nullptr, nullptr, Encoding::SOFTBANK_SHIFT_JIS,
            Language::JAPANESE, CompactEncDet::QUERY_CORPUS, true, &bytes_consumed, &is_reliable);
        if (detectedEncoding != JAPANESE_SHIFT_JIS && detectedEncoding != ASCII_7BIT &&
            detectedEncoding != ISO_8859_1 && detectedEncoding != DOCOMO_SHIFT_JIS &&
            detectedEncoding != KDDI_SHIFT_JIS && detectedEncoding != SOFTBANK_SHIFT_JIS)
        {
            auto msg = QCoreApplication::translate(
                           "Misc", "The text encoding of file \"%1\" seems like %2 instead of Shift-JIS.\n"
            ...
            //             if (!is_reliable){
            //                 msg += QCoreApplication::translate("Misc", "\n(Detecting result seems unreliable, so
            //                 check twice before answer this dialog.)");
            //             }
